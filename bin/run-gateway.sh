...
export CLASSPATH=${CLASSPATH}:gateway-core/target/gateway-core-1.0-SNAPSHOT.jar
...
retVal=$?
if [ $retVal -ne 0 ]; then
    echo "Gateway failed to start.  Ensure your CLASSPATH includes gateway-core-1.0-SNAPSHOT.jar and any interceptors defined under the pluginClass in your application.yaml" ${CLASSPATH}
fi
exit $retVal