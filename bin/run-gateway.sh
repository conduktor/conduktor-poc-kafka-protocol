...
export CLASSPATH=${CLASSPATH}:gateway-core/target/gateway-core-0.7.0-SNAPSHOT.jar
...
if [ $retVal -ne 0 ] && [ $retVal -ne 130 ] ; then
    echo "Gateway failed to start.  Ensure your CLASSPATH includes gateway-core-0.7.0-SNAPSHOT.jar and any interceptors defined under the pluginClass in your application.yaml" ${CLASSPATH}
fi
exit $retVal
